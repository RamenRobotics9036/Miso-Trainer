...
public class SwerveSimTest {
        ...
        try {
            ...
            assertAll(
                    () -> assertEquals(-0.033 / m_DistanceErrorFactor,
                            m_swerve.m_frontLeft.getPosition().distanceMeters, 0.001,
                            "FL m"),
                    () -> assertEquals(0.033 / m_DistanceErrorFactor,
                            m_swerve.m_frontRight.getPosition().distanceMeters, 0.001,
                            "FR m"),
                    () -> assertEquals(-0.033 / m_DistanceErrorFactor, m_swerve.m_backLeft.getPosition().distanceMeters,
                            0.001, "BL m"),
                    () -> assertEquals(0.033 / m_DistanceErrorFactor, m_swerve.m_backRight.getPosition().distanceMeters,
                            0.001,
                            "BR m"));
            ...
            assertEquals(3.090 / m_DistanceErrorFactor, m_swerve.speeds.omegaRadiansPerSecond, 0.001,
                    "chassis speed omega");
            ...
            assertAll(
                    ...
                    () -> assertEquals(0.062 / m_DistanceErrorFactor, m_swerve.getPose().getRotation().getRadians(),
                            0.001, "pose rot"));

            // look at the gyro, note NWU/NED difference, also one is rad the other deg
            assertAll(
                    () -> assertEquals(0.062 / m_DistanceErrorFactor, m_swerve.m_gyro.getRotation2d().getRadians(),
                            0.001, "gyro rotation"),
                    () -> assertEquals(-3.542 / m_DistanceErrorFactor, m_swerve.m_gyro.getAngle(), 0.001,
                            "gyro angle"));
            ...
            assertAll(
                    ...
                    () -> assertEquals(0.062 / m_DistanceErrorFactor, finalPose.getRotation().getRadians(), 0.001,
                            "estimate rot"));
        } finally {
            m_swerve.close(); // release the HAL stuff
        }
    ...
    void rotatedTranslationTest() {
        ...
        try {
            ...
            // each wheel should have moved the same as the displacement case above
            assertAll(
                    () -> assertEquals(0.033 / m_DistanceErrorFactor, m_swerve.m_frontLeft.getPosition().distanceMeters,
                            0.001,
                            "FL m"),
                    () -> assertEquals(0.033 / m_DistanceErrorFactor,
                            m_swerve.m_frontRight.getPosition().distanceMeters, 0.001,
            ...
            // relative.
            assertEquals(1.667 / m_DistanceErrorFactor, m_swerve.speeds.vxMetersPerSecond, 0.001, "chassis speed x");
            assertEquals(0, m_swerve.speeds.vyMetersPerSecond, 0.001, "chassis speed y");
            ...
            assertAll(
                    () -> assertEquals(0, m_swerve.getPose().getX(), 0.001, "pose x"),
                    () -> assertEquals(0.033 / m_DistanceErrorFactor, m_swerve.getPose().getY(), 0.001, "pose y"), // <=
                                                                                                                   // broken?
                    () -> assertEquals(Math.PI / 2, m_swerve.getPose().getRotation().getRadians(), 0.001, "pose rot"));
